#!/usr/bin/env bash
# Litchi – interactive install script
#
...
echo ""
bold "╔══════════════════════════════════════════════════════════╗"
bold "║         Litchi – open-source AI-native project mgmt     ║"
bold "╚══════════════════════════════════════════════════════════╝"
echo ""
...
echo "  database. If you are connecting to an existing Litchi database you MUST"
...
DB_CHOICE=""
ask_choice DB_CHOICE "How should Litchi store data?" \
...
if [[ -n "$SCALE_POSTGRES" ]]; then
    ...
    info "likely already handles this). To have Litchi back it up instead, set"
...
ADDRESS=""
ask ADDRESS "Domain name (recommended) or IP address Litchi will be accessible at" "localhost"
...
if [[ "$USE_HTTPS" == "yes" ]]; then
    ...
    SITE_ADDRESS=":80"
    ask GATEWAY_PORT "Gateway port (the port Litchi will be accessible on)" "80"
    ...
    ask PUBLIC_URL "Public URL (full URL where Litchi will be accessible, no trailing slash)" "$_DEFAULT_PUBLIC_URL"
fi
...
START="yes"
yes_no START "Pull images and start Litchi now?" "y"

if [[ "$START" != "yes" ]]; then
    echo ""
    warn "Installation files are ready. Start Litchi manually with:"
    echo ""
    bold "  cd $(pwd)"
